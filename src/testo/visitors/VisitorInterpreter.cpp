...
asio::awaitable<void> VisitorInterpreter::visit() {
	TRACE();
	...
	for (size_t current_test_run_index = 0; current_test_run_index < tests_runs.size(); ++current_test_run_index) {
		...
		if (skip_test) {
			co_await delete_parents_hypervisor_snapshots_if_needed(test_run->test);
			reporter.skip_test();
			continue;
		}

		co_await visit_test(test_run->test);
...
asio::awaitable<void> VisitorInterpreter::delete_parents_hypervisor_snapshots_if_needed(const std::shared_ptr<IR::Test>& test) {
	TRACE();
...
asio::awaitable<void> VisitorInterpreter::restore_parents_controllers_if_needed(const std::shared_ptr<IR::Test>& test) {
	TRACE();
	for (auto parent: test->parents) {
		for (auto controller: parent->get_all_controllers()) {
...
asio::awaitable<void> VisitorInterpreter::install_new_controllers_if_needed(const std::shared_ptr<IR::Test>& test) {
	TRACE();
	for (auto controller: test->get_all_controllers()) {
...
asio::awaitable<void> VisitorInterpreter::create_all_controllers_snapshots(const std::shared_ptr<IR::Test>& test) {
	TRACE();
...
asio::awaitable<void> VisitorInterpreter::visit_test(const std::shared_ptr<IR::Test>& test) {
	TRACE();
	...
	try {
		...
		co_await restore_parents_controllers_if_needed(test);
		create_networks_if_needed(test);
		co_await install_new_controllers_if_needed(test);
		...
		resume_parents_vms(test);
		{
			...
			current_test = test;
			co_await visit_command_block(test->ast_node->cmd_block);
		}
		suspend_all_vms(test);

		co_await delete_parents_hypervisor_snapshots_if_needed(test);
		co_await create_all_controllers_snapshots(test);
...
asio::awaitable<void> VisitorInterpreter::visit_command_block(const std::shared_ptr<AST::Block<AST::Cmd>>& block) {
	for (auto command: block->items) {
		co_await visit_command(command);
	}
}

asio::awaitable<void> VisitorInterpreter::visit_command(const std::shared_ptr<AST::Cmd>& cmd) {
	if (auto p = std::dynamic_pointer_cast<AST::RegularCmd>(cmd)) {
		co_await visit_regular_command({p, stack});
...
asio::awaitable<void> VisitorInterpreter::visit_regular_command(const IR::RegularCommand& regular_command) {
	if (auto current_controller = IR::program->get_machine_or_null(regular_command.entity())) {
		...
		co_await visitor.visit_action(regular_command.ast_node->action);
...
asio::awaitable<void> VisitorInterpreter::visit_macro_call(const IR::MacroCall& macro_call) {
	reporter.macro_command_call(macro_call);
	co_await macro_call.visit_interpreter<AST::Cmd>(this);
}

asio::awaitable<void> VisitorInterpreter::visit_macro_body(const std::shared_ptr<AST::Block<AST::Cmd>>& macro_body) {
	co_await visit_command_block(macro_body);
}
