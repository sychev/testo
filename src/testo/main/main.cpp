...
int do_main(int argc, char** argv) {
	...
	auto run_spec = "run options" % (
		...
		(option("--report_folder") & value("/path/to/folder or host:port", run_args.report_folder)) % "Save the report in a specified folder (host:port for native_remote format)",
		...
		(option("--nn_server") & value("host:port", run_args.nn_server_endpoint)) % "host:port of the nn_server (default is 127.0.0.1:8156)",
